...
from PyQt5.QtWidgets import QApplication, QMainWindow, QLabel, QGraphicsScene, QGraphicsView, QGraphicsPixmapItem, QVBoxLayout, QWidget, QTextEdit, QPushButton
...
class TachieDisplay(QMainWindow):
    def __init__(self):
        ...
        self.character_image = QImage(self.image_path).convertToFormat(QImage.Format_RGBA8888)

        image_width, image_height = self.character_image.width(), self.character_image.height()
        ...
        self.dialog_widget.setGeometry(
            int(self.dialog_x), int(self.dialog_y), int(self.dialog_width), int(self.dialog_height)
        )
        self.dialog_widget.setStyleSheet("background-color: white; border: 1px solid gray;")
        ...
        self.dialog_text.setFixedWidth(self.dialog_width - 20)
        self.dialog_layout.addWidget(self.dialog_text)

        self.dialog_text.returnPressed.connect(self.send_text)
