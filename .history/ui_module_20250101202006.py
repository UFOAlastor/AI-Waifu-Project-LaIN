...
class TachieDisplay:
    def __init__(self):
        ...
        self.alpha_surface.fill((0, 0, 0, 0))  # 设置完全透明背景
        ...
        self.offset_x = 0
        self.offset_y = 0
        self.dragging_character = False  # 是否拖动人物立绘
        ...
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                if event.button == 1:  # 左键点击
                    if self.dialog_rect.collidepoint(event.pos):  # 如果点击了文本框
                        # 允许拖动对话框
                        self.offset_x = event.pos[0] - self.dialog_rect.x
                        self.offset_y = event.pos[1] - self.dialog_rect.y
                    elif self.character_rect.collidepoint(event.pos):  # 如果点击了人物立绘
                        # 允许拖动人物立绘
                        self.dragging_character = True
                        self.offset_x = event.pos[0] - self.character_rect.x
                        self.offset_y = event.pos[1] - self.character_rect.y
                    elif self.close_button.collidepoint(event.pos):
                        self.running = False  # 关闭按钮点击
            elif event.type == MOUSEBUTTONUP:
                if event.button == 1:
                    # 释放鼠标时停止拖动人物立绘
                    self.dragging_character = False
            elif event.type == MOUSEMOTION:
                if self.dragging_character:
                    # 拖动人物立绘
                    self.character_rect.x = event.pos[0] - self.offset_x
                    self.character_rect.y = event.pos[1] - self.offset_y
                elif pygame.mouse.get_pressed()[0]:  # 左键按住时拖动对话框
    ...
    def render(self):
        ...
        # 绘制对话框
        self.draw_rounded_rect(self.screen, self.dialog_rect, 20, (255, 255, 255, 200))  # 圆角对话框
    ...
    def draw_rounded_rect(self, surface, rect, radius, color):
        """绘制圆角矩形"""
