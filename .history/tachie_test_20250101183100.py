...
class TachieDisplay:
    def __init__(self, root):
        # 加载配置文件
        self.settings = self.load_config("config.json")

        # 初始化窗口
        window_settings = self.settings["window"]
        root.geometry(f'{window_settings["width"]}x{window_settings["height"]}')
        root.overrideredirect(True)
        root.attributes("-transparentcolor", window_settings["transparent_color"])
        root.configure(bg=window_settings["transparent_color"])

        # 加载立绘图像
        char_settings = self.settings["character"]
        self.character_image = Image.open(char_settings["image_path"]).convert("RGBA")
