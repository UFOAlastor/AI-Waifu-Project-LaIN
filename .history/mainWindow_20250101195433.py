...
class TachieDisplay:
    def __init__(self, root):
        ...
        # 初始化模型（假设你有一个模型类）
        self.model = None  # 这里可以实例化你的模型对象

    def send_text(self, event=None, text=None):
        """处理回车键发送文本"""
        if text is None:
            # 如果没有传入文本，使用原来的方式获取文本
    ...
    def load_settings(self, file_path):
        """加载设置文件"""
        try:
            with open(file_path, "r", encoding="utf-utf8") as f:
                return json.load(f)
    ...
    def main(self):
        """自定义主程序，接入模型和事件循环"""
        # 你可以通过 self.root.after 来调用定时任务更新对话框内容
        self.root.after(1000, self.run_model)
        self.root.mainloop()
