...
class TachieDisplay(QMainWindow):
    ...
    def __init__(self):
        ...
        # 设置 QPlainTextEdit 使其能接收鼠标点击事件
        self.dialog_text = QPlainTextEdit(self)
    ...
    def eventFilter(self, obj, event):
        ...
        if obj == self.dialog_text and event.type() == QEvent.MouseButtonPress:
            if event.button() == Qt.LeftButton:
                self.mousePressEvent(event)  # 捕获点击并调用 mousePressEvent
                return True  # 表示事件已处理，不再传播
        elif obj == self.dialog_text and event.type() == QEvent.KeyPress:
            # 捕获回车键事件
            key_event = event
