...
class TachieDisplay:
    def __init__(self, root):
        ...
        window_width = self.settings.get('window_width', 600)
        window_height = self.settings.get('window_height', 600)
        dialog_x =  self.settings.get('dialog_x', window_width*0.1)
        dialog_y =  self.settings.get('dialog_y', window_height*0.5)
        dialog_width = self.settings.get('dialog_width', window_width*0.8)
        dialog_height = self.settings.get('dialog_height', window_height*0.2)
        dialog_opacity = self.settings.get('dialog_opacity', 0.8)
        ...
        self.dialog_label = tk.Label(
            self.dialog_frame, text="绫", font=("Arial", 14), bg="white", anchor="w"
        )
        ...
        # 设置窗口透明度
        self.root.attributes("-alpha", dialog_opacity)  # 默认透明度 90%
        ...
        self.close_button = tk.Button(
            self.dialog_frame, text="×", command=self.root.destroy, bg="red", fg="white", font=("Arial", 12, "bold")
        )
        self.close_button.place(x=self.settings.get("dialog_width", 600) - 30, y=10, width=30, height=30)
    ...
    def on_resize(self, event):
        ...
        # 调整关闭按钮位置
        dialog_width = self.settings.get("dialog_width", 600)
        dialog_height = self.settings.get("dialog_height", 150)
        self.close_button.place(x=width - dialog_width + 10, y=height - dialog_height + 10)
