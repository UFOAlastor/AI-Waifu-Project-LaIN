...
class TachieDisplay(QMainWindow):
    def __init__(self):
        ...
        # 监听文本框内容变化
        self.dialog_text.textChanged.connect(self.on_text_changed)

        # 创建发送按钮
        self.send_button = QPushButton("发送", self.dialog_widget)
        self.send_button.setStyleSheet("background-color: #4CAF50; color: white; font: bold 12pt Arial; border: none; border-radius: 5px;")
        self.send_button.clicked.connect(self.send_text)  # 点击按钮发送文本
        self.dialog_layout.addWidget(self.send_button)
    ...
    def on_text_changed(self):
        # 监听文本框内容的变化，如果回车键被按下，就发送文本
        text = self.dialog_text.toPlainText().strip()
        if text.endswith('\n'):
            self.send_text()  # 自动发送文本
