...
class VitsSpeaker:
    ...
    def get_audio_stream(text, speaker_id=None, lang="zh", format="wav", length=1.0):
        """发送请求并获取音频流"""
        speaker_id = speaker_id or VitsSpeaker.SPEAKER_ID  # 如果未传入speaker_id，使用默认的SPEAKER_ID
        params = {
            "text": text,
            "id": speaker_id,
            "lang": lang,
    ...
    def synthesize_and_play(text, speaker_id=None, lang="zh", format="wav", length=1.0):
        """输入文本，生成并播放音频"""
        try:
            audio_data = VitsSpeaker.get_audio_stream(text, speaker_id, lang, format, length)
