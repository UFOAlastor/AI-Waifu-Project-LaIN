...
def parse_bot_reply(reply: str, delimiter: str = "|||"):
    ...
    if not match:
        raise ValueError(f"输入字符串格式不正确，应为 {{表情}}{delimiter}{{中文}}{delimiter}{{日语}}")
...
def generate_bot_reply(expression: str, chinese: str, japanese: str, delimiter: str = "|||"):
    """
    生成符合分隔符格式的 Bot 回复。
...
# 生成新回复
new_reply = generate_bot_reply("微笑", "这是新的中文", "これは新しい日本語です")
print("生成的回复:", new_reply)
