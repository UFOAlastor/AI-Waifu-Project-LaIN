...
class ModelResponseThread(QThread):
    response_ready = pyqtSignal(str)  # 模型回应完成的信号

    def __init__(self, chat_model, input_text):
        super().__init__()
...
class TachieDisplay(QMainWindow):
    ...
    def display_text(self, content=None, is_non_user_input=False, is_generating=False):
        """
        显示文本内容，is_non_user_input 为 True 时表示显示模型返回的内容；
        is_generating 为 True 时表示等待生成中，显示动态省略号。
        """
        self.is_non_user_input = is_non_user_input
        self.dialog_text.clear()  # 清空对话框

        if is_generating:
            self.current_dots = 0  # 当前省略号数量
            self.timer = QTimer(self)
            self.timer.timeout.connect(self.show_generating_dots)
            self.timer.start(500)  # 每隔 500 毫秒更新一次省略号
        else:
            self.dialog_text.setPlainText("")  # 清空文本框内容
            self.current_char_index = 0  # 当前字符索引
            self.content = content or ""  # 存储要显示的完整文本

            # 设置定时器，模拟逐个字符的显示
            self.timer = QTimer(self)
            self.timer.timeout.connect(self.on_typing)
            self.timer.start(100)  # 每个字符之间的延迟 100 毫秒

    def show_generating_dots(self):
        """
        动态显示省略号。
        """
        dots = "." * self.current_dots
        self.dialog_text.setPlainText(f"生成中{dots}")
        self.current_dots = (self.current_dots + 1) % 4  # 动态显示 0-3 个点
    ...
    def on_typing(self):
        ...
        if self.current_char_index < len(self.content):
            ...
            self.timer.stop()  # 停止定时器，文本显示完成

    def on_text_received(self, input_text):
        """
        当收到用户输入时，调用模型生成回复并更新 UI。
        """
        if input_text:
            self.display_text(is_generating=True)  # 显示动态省略号

            # 启动后台线程获取模型的回复
            self.response_thread = ModelResponseThread(self.chat_model, input_text)
            self.response_thread.response_ready.connect(self.on_response_ready)
            self.response_thread.start()

    def on_response_ready(self, reply_text):
        """
        模型生成回复完成，更新 UI。
        """
        if self.timer:  # 停止动态省略号的显示
            self.timer.stop()
        self.display_text(content=reply_text, is_non_user_input=True)
