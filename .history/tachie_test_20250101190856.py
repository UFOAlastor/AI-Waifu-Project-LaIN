...
class TachieDisplay:
    def __init__(self, root):
        ...
        self.character_image = self.character_image.resize(
            (
                int(self.character_image.width * self.scale),
                int(self.character_image.height * self.scale),
            )
        )
        ...
        self.close_button.place(x=(self.window_width - self.dialog_width) // 2, y=self.window_height - (self.window_height - self.dialog_height) // 3, width=30, height=30)
    ...
    def on_resize(self, event):
        ...
        # 调整关闭按钮位置
        self.close_button.place(x=(width - self.dialog_width) // 2, y=height - (height - self.dialog_height) // 3)
